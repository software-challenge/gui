...
class BlokusController : Controller() {
    ...
    val undeployedPieces: Map<Color, ObservableValue<Collection<PieceShape>>> = EnumMap(
        Color.values().associateWith { color ->
            nonNullObjectBinding(gameState, gameState) {
                value?.undeployedPieceShapes(color) ?: PieceShape.values().toList()
            }
        })
    ...
    init {
        ...
        subscribe<HumanMoveRequest> { event ->
            val state: GameState = event.gameState as GameState
            val moves = EnumMap(
